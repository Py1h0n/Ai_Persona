...
const constructImagePrompt = (personaDNA: PersonaDNA, sceneConfig: SceneConfig): string => {
    return `
      **Primary Subject:** A single person defined by this personality: "${personaDNA.personality}".

      **Scene Description:** The person is captured in a candid moment, performing the action: "${sceneConfig.action}".
      The setting is "${sceneConfig.location}", which should complement their primary environment: "${personaDNA.environment}".
      The time of day is "${sceneConfig.timeOfDay}".

      **Art Direction & Photography Style:**
      - **Overall Aesthetic:** The image must strictly adhere to a "${personaDNA.aesthetic}" aesthetic. This is the most important instruction.
      - **Camera & Composition:** The shot should be from a "${sceneConfig.cameraAngle}" angle.
      - **Realism:** Emphasize photorealism. The final image should look like a real, candid photograph, not a staged or AI-generated picture. Avoid artificial poses, perfect lighting, or overly clean compositions unless specified by the aesthetic. Capture imperfections.
      ...
      **Final Instruction:** Generate a single, photorealistic image based on all the above criteria. The aesthetic is paramount.
    `;
};
